                     ...
                     const int numbuffers)
{
  ...
  /* support for other depths must include bpp convertion */
  //assert(depth == 16); 
  assert(numbuffers >= 1);
  ...
  for (i = 0; i < numbuffers; i++) {
    zb = ZB_open(xsize, ysize, mode, 0, NULL, NULL, framebuffers ? framebuffers[i] : NULL);
    if (zb == NULL) {
      fprintf(stderr, "Error while initializing Z buffer\n");
      exit(1);
    }
    context->zbs[i] = zb;
    context->framebuffers[i] = zb->pbuf;
  }
  if (++buffercnt == 1) {
             ...
             void **framebuffers)
{
  int i;
  for (i = 0; i < context->numbuffers; i++) {
    ZB_resize(context->zbs[i], framebuffers ? framebuffers[i] : NULL, xsize, ysize);
  }
}

void
ostgl_copy_framebuffer(ostgl_context *oscontext, const int idx, void *buf, int linesize)
{
  assert(idx < oscontext->numbuffers);
  ZB_copyFrameBuffer(oscontext->zbs[idx], buf, linesize);
}
